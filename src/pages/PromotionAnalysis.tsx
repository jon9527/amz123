...
const PromotionAnalysis: React.FC = () => {
    ...
    const calc = useMemo(() => {
        ...
        // 3. First Mile (头程物流) - Separated
        const firstMile = inputs.shippingUSD;

        // 4. FBA配送费 - FBA fee alone
        const fbaFee = inputs.fbaFee;

        // 5. 仓储杂费 - Combined: miscFee + agedInventoryFee + storageFee (match ProfitCalculator)
        const agedInventoryFee = inputs.agedInventoryFee || 0;
        const storageMisc = (inputs.miscFee || 0) + agedInventoryFee + (inputs.storageFee || 0);

        // 6. Commission (Dynamic)
        // Use saved rate or default 15%
        const commRate = pb.commRate || 0.15;
        const commission = simPrice * commRate;

        // 7. Returns (Dynamic & Precise)
        ...
        // 9. Total Cost
        const totalCost = purchaseCost + firstMile + fbaFee + storageMisc + returnsCost + commission + adSpend;
        ...
        const vStorageMisc = storageMisc * vol;
        ...
        // *** Waterfall Data Preparation (Match ProfitCalculator.tsx exactly) ***
        const p1 = vTotalSales;
        const p2 = r2(p1 - vPurchase);
        const p3 = r2(p2 - vFirstMile);
        const p4 = r2(p3 - vFba);
        const p5 = r2(p4 - vStorageMisc);
        ...
        const waterfallData = [
            ...
            { name: '采购成本', val: -vPurchase, range: [p2, p1], color: '#3b82f6' },       // blue-500
            { name: '头程物流', val: -vFirstMile, range: [p3, p2], color: '#0ea5e9' },      // sky-500
            { name: 'FBA配送费', val: -vFba, range: [p4, p3], color: '#9333ea' },           // purple-600
            { name: '仓储杂费', val: -vStorageMisc, range: [p5, p4], color: '#818cf8' },    // indigo-400
            { name: '销售佣金', val: -vCommission, range: [p6, p5], color: '#f97316' },     // orange-500
            { name: '退货损耗', val: -vReturns, range: [p7, p6], color: '#f43f5e' },        // rose-500
            { name: '广告成本', val: -vAds, range: [p8, p7], color: '#fbbf24' },            // amber-400
            { name: '实际利润', val: vProfit, range: [0, vProfit], color: '#10b981' }       // emerald-500
        ];
        ...
        return {
            unit: {
                ...
                netProfit,
                netMargin,
                baseCostPerUnit: purchaseCost + firstMile + fbaFee + storageMisc + returnsCost, // EXCLUDE commission
        ...
        <div ref={contentRef} className="p-8 space-y-12 max-w-[1800px] mx-auto animate-in fade-in duration-500 pb-24">
            ...
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h2 className="text-2xl font-black text-white">盈亏沙盘</h2>
                        <p className="text-zinc-500 text-xs uppercase tracking-widest font-bold">Profit Analysis Sandbox & Budget Simulation</p>
                    </div>
                </div>
                ...
                <div className="flex items-center gap-3">
                    ...
                    <ProductSchemeDropdown
                        ...
                        onSelectModel={(model) => handleModelChange(model.id)}
                        accentColor="blue"
                    />
                </div>
            </div>
                ...
                <>
                    ...
                    <div className='space-y-4'>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-9 gap-4 w-full">
                            ...
                            <CostCard label="③ 头程物流" value={calc.unit.firstMile} subValue={`${((calc.unit.firstMile / calc.unit.totalSales) * 100).toFixed(1)}%`} color="sky" exchangeRate={savedModels.find(m => m.id === selectedModelId)?.inputs.exchangeRate} />
                            <CostCard label="④ FBA配送费" value={calc.unit.fbaFee} subValue={`${((calc.unit.fbaFee / calc.unit.totalSales) * 100).toFixed(1)}%`} color="purple" exchangeRate={savedModels.find(m => m.id === selectedModelId)?.inputs.exchangeRate} />
                            <CostCard label="⑤ 仓储杂费" value={calc.unit.storageMisc} subValue={`${((calc.unit.storageMisc / calc.unit.totalSales) * 100).toFixed(1)}%`} color="indigo" exchangeRate={savedModels.find(m => m.id === selectedModelId)?.inputs.exchangeRate} />
                    ...
                    <div className='space-y-4'>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-9 gap-4 w-full opacity-90">
                            ...
                            <CostCard label="③ 头程物流" value={calc.volume.firstMile} subValue={`${((calc.volume.firstMile / calc.volume.totalSales) * 100).toFixed(1)}%`} color="sky" exchangeRate={savedModels.find(m => m.id === selectedModelId)?.inputs.exchangeRate} />
                            <CostCard label="④ FBA配送费" value={calc.volume.fbaFee} subValue={`${((calc.volume.fbaFee / calc.volume.totalSales) * 100).toFixed(1)}%`} color="purple" exchangeRate={savedModels.find(m => m.id === selectedModelId)?.inputs.exchangeRate} />
                            <CostCard label="⑤ 仓储杂费" value={calc.volume.storageMisc} subValue={`${((calc.volume.storageMisc / calc.volume.totalSales) * 100).toFixed(1)}%`} color="indigo" exchangeRate={savedModels.find(m => m.id === selectedModelId)?.inputs.exchangeRate} />
