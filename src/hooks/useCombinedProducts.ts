 ...
 * 
 * Optimized: Loads localStorage data asynchronously to avoid blocking main thread.
 */
export const useCombinedProducts = () => {
    const { products: contextProducts } = useProducts();
    const [skuProducts, setSkuProducts] = useState<ProductSpec[]>([]);
    ...
    useEffect(() => {
        ...
        try {
            ...
            if (saved) {
                // Use setTimeout to ensure this runs in next tick, fully unblocking render
                setTimeout(() => {
                    try {
                        ...
                        setSkuProducts(mapped);
                    } catch (parseError) {
                        console.error('Failed to parse sku groups async', parseError);
                    }
                }, 0);
            }
        } catch (e) {
